...
pub struct Source {
    ...
    pub file: &'static str,
    /// True only for the invented onboarding corpus. Every other source is an
    /// unmodified public capture, and the console says which is which so the
    /// distinction survives being demonstrated rather than living in a README.
    pub synthetic: bool,
}
...
pub const SOURCES: &[Source] = &[
    ...
    // Not a capture. Generated by `tools/make_demo_source.py` so that onboarding
    // an unseen device can be demonstrated on a machine whose every real corpus
    // is already claimed by a shipped pack. Excluded from every measurement.
    Source {
        id: "apx-synthetic",
        label: "Aperture APX NGFW",
        category: "Firewall",
        origin: "SYNTHETIC — not a capture",
