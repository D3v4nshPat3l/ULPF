...
pub struct Source {
    ...
    pub file: &'static str,
    /// True only for the generated onboarding corpus.
    ///
    /// Not cosmetic: this is the flag `tools/measure_coverage.py` asserts on,
    /// so a generated corpus cannot reach a published coverage figure. The UI
    /// no longer draws attention to it, which makes the assertion the thing
    /// actually holding the line rather than a label someone can overlook.
    pub synthetic: bool,
}
...
pub const SOURCES: &[Source] = &[
    ...
    // Generated by `tools/make_demo_source.py`, because onboarding an unseen
    // device cannot be demonstrated on a machine whose every real corpus is
    // already claimed by a shipped pack. The `synthetic` flag is what keeps it
    // out of `tools/measure_coverage.py`, which asserts on its absence, so no
    // published figure can ever include it.
    Source {
        id: "apx-synthetic",
        label: "Aperture APX NGFW",
        category: "Firewall",
...
impl Simulator {
    ...
    /// Turn every source that is actually on disk on at one rate.
    ///
    /// Returns the ids started. A source with no corpus behind it is skipped
    /// rather than reported as running: the simulator's rule is that a stream
    /// is real capture data or it does not exist, and "start all" must not be
    /// the one path that quietly invents an exception.
    pub fn start_all(&mut self, eps: u64) -> Vec<String> {
        let candidates: Vec<(&str, bool)> = SOURCES
            .iter()
            .map(|source| (source.id, self.path_for(source).is_file()))
            .collect();

        let mut started = Vec::new();
        for (id, present) in candidates {
            if !present || self.streams.contains_key(id) {
                continue;
            }
            if self.start(id, eps).is_ok() {
