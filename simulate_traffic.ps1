...
Write-Host "Starting live traffic simulation for ULPF..."

while ($true) {
    if ((Get-Random -Minimum 1 -Maximum 100) -le 80) {
        $log = $KnownLogs | Get-Random
    } else {
        $log = $UnknownLogs | Get-Random
    }
    ...
    $sleepMs = Get-Random -Minimum 100 -Maximum 500
